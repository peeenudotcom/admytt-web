...
const modules: ModuleDetail[] = [
  { id: "leads", icon: "LP", bg: "#EEF2FF", fg: "#4F46E5", title: "Lead & pipeline management", outcome: "Capture, qualify, and route every enquiry the moment it arrives.", capabilities: ["Capture from web forms, WhatsApp, email, and walk-ins", "Auto-assign by branch, counsellor, and rule", "Lead scoring that surfaces high-intent students", "A pipeline built around education stages, not generic deals"] },
  { id: "counselling", icon: "CF", bg: "#EEF2FF", fg: "#4F46E5", title: "Counselling & follow-ups", outcome: "Keep every conversation and never miss the next action.", capabilities: ["Full conversation history on each record", "Reminders and next actions that follow the student", "Course recommendations captured in context", "Today's prioritised follow-up list per counsellor"] },
  ...
  { id: "documents", icon: "DM", bg: "#FBE9E9", fg: "#B91C1C", title: "Document management", outcome: "See exactly what is missing, rejected, verified, and versioned.", capabilities: ["Readiness checklist per application", "Versioned uploads with review status", "Rejections flagged with a clear reason", "Private, workspace-scoped storage"] },
  ...
  { id: "visa", icon: "VV", bg: "#EEF2FF", fg: "#4F46E5", title: "Visa & visitor visa", outcome: "Run visa work with clear ownership and fewer missed requirements.", capabilities: ["Country-specific checklists and filing stages", "Document status tracked to each case", "Ownership and decisions recorded", "Visitor-visa workflows alongside student visas"] },
  { id: "partner-hub", icon: "PH", bg: "#EEF2FF", fg: "#4F46E5", title: "Partner Hub", outcome: "Scale a partner network without surrendering control.", capabilities: ["Onboard sub-agents with scoped access", "Clear lead ownership and compliance", "Agreements, resources, and courses in one place", "Commission tracking per partner"] },
    ...
    <PageShell>
      ...
      <Band bg="#fff" border={false}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }} data-platform="1">
          {["Enquiry & qualification", "Counselling & course selection", "Applications & documents", "Finance, visa & enrolment"].map((t, i) => (
            <div key={t} style={{ background: "#F8FAFC", border: "1px solid #E9EEF6", borderRadius: 12, padding: 22 }}>
              <span style={badgeStyle("info")}>{`Step ${i + 1}`}</span>
              <div style={{ font: "700 15.5px var(--font-inter)", color: "#0F172A", margin: "13px 0 0" }}>{t}</div>
            </div>
          ))}
        </div>
      </Band>
      ...
      <Band bg="#F8FAFC">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {modules.map((m) => (
            <div key={m.id} id={m.id} style={{ scrollMarginTop: 90, background: "#fff", border: "1px solid #E9EEF6", borderRadius: 14, padding: 30, display: "grid", gridTemplateColumns: ".7fr 1.3fr", gap: 28 }} data-teamview="1">
              <div>
                <span style={{ display: "flex", width: 46, height: 46, borderRadius: 11, alignItems: "center", justifyContent: "center", font: "700 18px var(--font-inter)", background: m.bg, color: m.fg }}>{m.icon}</span>
                <h3 style={{ font: "700 21px/28px var(--font-inter)", color: "#0F172A", margin: "16px 0 8px" }}>{m.title}</h3>
                <p style={{ font: "400 15px/23px var(--font-inter)", color: "#475569" }}>{m.outcome}</p>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignContent: "start" }}>
                {m.capabilities.map((c) => (
                  <div key={c} style={{ display: "flex", alignItems: "flex-start", gap: 10, background: "#F8FAFC", border: "1px solid #EAEFF7", borderRadius: 10, padding: "12px 14px", font: "500 14px/20px var(--font-inter)", color: "#334155" }}>
        ...
        <div style={{ marginTop: 28 }}>
          <Link href="/ai" style={{ font: "700 15px var(--font-inter)", color: "#4F46E5", textDecoration: "none" }}>See how the AI Command Centre assists each module →</Link>
        </div>
      </Band>
