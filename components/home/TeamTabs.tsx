    ...
    <section id="teams" style={{ background: "#fff", padding: "92px 0" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 28px" }}>
        <div style={{ maxWidth: 680, marginBottom: 34 }}>
          <div style={{ font: "700 13px var(--font-inter)", color: "#2453D4", marginBottom: 12 }}>Built for each team</div>
          <h2 data-h2 style={{ font: "800 38px/45px var(--font-manrope)", letterSpacing: "-.02em", color: "#0B1230", marginBottom: 14 }}>One platform, reorganised around each role</h2>
          ...
          {teams.map((tm, i) => {
            ...
            return (
              <button
                ...
                aria-selected={on}
                onClick={() => setActive(i)}
                style={{ font: "700 14px var(--font-manrope)", background: on ? "#212177" : "#fff", color: on ? "#fff" : "#475569", border: `1px solid ${on ? "#212177" : "#E4EAF3"}`, borderRadius: 9, padding: "10px 18px", cursor: "pointer", transition: "all .15s" }}
              >
                {tm.label}
              </button>
            );
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, alignItems: "center", background: "#F6F8FC", border: "1px solid #E9EEF6", borderRadius: 16, padding: 36 }} data-teamview="1">
          <div>
            <h3 style={{ font: "700 25px/32px var(--font-manrope)", color: "#0B1230", marginBottom: 14 }}>{t.headline}</h3>
          ...
          <div style={{ background: "#fff", border: "1px solid #E4EAF3", borderRadius: 12, overflow: "hidden", boxShadow: "0 22px 46px -28px rgba(11,18,48,.3)" }}>
            <div style={{ background: "#0B1230", padding: "12px 16px", font: "700 12.5px var(--font-manrope)", color: "#fff" }}>{t.panelTitle}</div>
