...
export default function DemoSection() {
  return (
    <section id="demo" style={{ background: "linear-gradient(135deg,#212177 0%,#0B1230 100%)", padding: "92px 0", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 28px", position: "relative", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }} data-demoform="1">
        <div style={{ paddingTop: 8 }}>
          <h2 data-h2 style={{ font: "800 40px/47px var(--font-manrope)", letterSpacing: "-.02em", color: "#fff", marginBottom: 18 }}>See how adMYTT fits the way your consultancy works.</h2>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {steps.map((s) => (
              <div key={s.n} style={{ display: "flex", alignItems: "flex-start", gap: 13 }}>
                <span style={{ width: 28, height: 28, flexShrink: 0, borderRadius: 8, background: "rgba(255,255,255,.1)", color: "#5fd0a6", display: "flex", alignItems: "center", justifyContent: "center", font: "700 13px var(--font-manrope)" }}>{s.n}</span>
                <div>
                  <div style={{ font: "700 15px var(--font-manrope)", color: "#fff" }}>{s.title}</div>
