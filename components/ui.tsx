...
export function PageHero({
  ...
  return (
    <section style={{ background: "radial-gradient(120% 120% at 80% -10%,#EEF2FF 0%,#F8FAFC 55%)", borderBottom: "1px solid #EEF2F8" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "72px 28px 64px" }}>
        <div style={{ maxWidth: 760 }}>
          <div style={{ font: "700 13px var(--font-inter)", color: "#4F46E5", marginBottom: 14 }}>{eyebrow}</div>
          <h1 data-h1 style={{ font: "800 48px/56px var(--font-inter)", letterSpacing: "-.02em", color: "#0F172A", marginBottom: 18 }}>{title}</h1>
          <p style={{ font: "400 18.5px/29px var(--font-inter)", color: "#475569", marginBottom: 30 }}>{lead}</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
            <Link href={primary.href} style={{ font: "700 15.5px var(--font-inter)", color: "#fff", textDecoration: "none", background: "#4F46E5", padding: "15px 28px", borderRadius: 8, boxShadow: "0 10px 24px rgba(79,70,229,.3)" }}>{primary.label}</Link>
            {secondary && (
              <Link href={secondary.href} style={{ font: "700 15.5px var(--font-inter)", color: "#0F172A", textDecoration: "none", background: "#fff", border: "1px solid #DCE3EE", padding: "15px 26px", borderRadius: 8 }}>{secondary.label}</Link>
            )}
          </div>
        </div>
      </div>
...
export function SectionHeading({ eyebrow, title, lead }: { eyebrow?: string; title: string; lead?: string }) {
  return (
    <div style={{ maxWidth: 680, marginBottom: 40 }}>
      {eyebrow && <div style={{ font: "700 13px var(--font-inter)", color: "#4F46E5", marginBottom: 12 }}>{eyebrow}</div>}
      <h2 data-h2 style={{ font: "800 36px/44px var(--font-inter)", letterSpacing: "-.02em", color: "#0F172A", marginBottom: 14 }}>{title}</h2>
...
export function CheckList({ items, color = "#4F46E5" }: { items: string[]; color?: string }) {
  return (
...
export function CtaBand({
  ...
  return (
    <section style={{ background: "linear-gradient(135deg,#4F46E5 0%,#0F172A 100%)", padding: "80px 0", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 28px", position: "relative", textAlign: "center" }}>
        <h2 data-h2 style={{ font: "800 36px/44px var(--font-inter)", letterSpacing: "-.02em", color: "#fff", marginBottom: 16 }}>{title}</h2>
        <p style={{ font: "400 18px/28px var(--font-inter)", color: "#c2cbe6", marginBottom: 30 }}>{body}</p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <Link href="/book-a-demo" style={{ font: "700 15.5px var(--font-inter)", color: "#fff", textDecoration: "none", background: "#4F46E5", padding: "15px 28px", borderRadius: 8, boxShadow: "0 10px 24px rgba(79,70,229,.3)" }}>Book a demo</Link>
          <a href={site.appUrl} style={{ font: "700 15.5px var(--font-inter)", color: "#fff", textDecoration: "none", background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.2)", padding: "15px 26px", borderRadius: 8 }}>Log in</a>
        </div>
      </div>
    </section>
  );
